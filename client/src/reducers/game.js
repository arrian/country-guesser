...
const game = (state = { players: [], guesses: [], giveUp: false, currentGuess: '', sendingGuess: false, sendingGiveUp: false, guessText: '', name: '' }, action) => {
	switch(action.type) {
	case 'UPDATE_WORLD':
		return Object.assign({}, state, { players: action.world.players, guesses: action.world.guesses.filter(guess => guess.correct && !guess.duplicate), name: action.world.name });
	...
	case 'UPDATE_GUESSES':
		return Object.assign({}, state, { sendingGuess: false, guesses: action.guesses.filter(guess => guess.correct && !guess.duplicate), guessText: '' });
